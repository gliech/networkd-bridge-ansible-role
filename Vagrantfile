...
#   { name: 'debian', box: 'debian/testing64' },
  {
    ...
    ip:   '10.10.10.10',
    hostvars: {
      networkd_bridge_ip_address: '10.10.10.10/24'
    }
  },
...
provisioner_box = 'archlinux/archlinux'
dhcp_box = 'rockylinux/8'
...
guests.each do |guest|
    ...
    if guest.has_key?(:ip) and guest[:ip]
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    guests.each_with_index do |guest, index|
        ...
        config.vm.define guest[:vagrant_name] do |machine|
            ...
            elsif guest.has_key?(:ip) and !guest[:ip]
                machine.vm.network 'private_network',
                    auto_config: false,
                    virtualbox__intnet: "vagrant-#{project_name}-network"
            else
            ...
            machine.vm.provider 'virtualbox' do |vbox|
                ...
                vbox.customize ["modifyvm", :id, "--nicpromisc1", "allow-all", "--nicpromisc2", "allow-all"]
            end
